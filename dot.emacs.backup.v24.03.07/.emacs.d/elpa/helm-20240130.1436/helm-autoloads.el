...
If called interactively, enable Helm-Adaptive mode if ARG is positive,
...
If called interactively, enable Helm-Epa mode if ARG is positive, and
...
If called interactively, enable Helm-Ff-Icon mode if ARG is positive,
and disable it if ARG is zero or negative.  If called from Lisp, also
enable the mode if ARG is omitted or nil, and toggle it if ARG is
`toggle'; disable the mode otherwise.
...
(if (fboundp 'register-definition-prefixes) (register-definition-prefixes "helm-packages" '("helm-packages-")))
...
If called interactively, enable Helm-Top-Poll mode if ARG is positive,
and disable it if ARG is zero or negative.  If called from Lisp, also
enable the mode if ARG is omitted or nil, and toggle it if ARG is
`toggle'; disable the mode otherwise.
